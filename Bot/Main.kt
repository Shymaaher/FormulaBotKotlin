...
class SimpleBot : TelegramLongPollingBot() {
     ...
     */
    private val activeUsers = mutableSetOf<Long>() // Хранилище активных пользователей
        ...
        if (update != null && update.hasMessage() && update.message.hasText()) {
            ...
            if (messageText == "/start") {
                if (activeUsers.contains(chatId)) {
                    sendResponse(
                        chatId,
                        "Бот уже активен. Воспользуйтесь встроенным меню!",
                        showMainButtons()
                    )
                } else {
                    ...
                    sendResponse(
                        chatId,
                        "Добро пожаловать в 2024 Formula 1 One World Championship - Календарь. Воспользуйтесь встроенным меню, чтобы продолжить пользоваться ботом!",
                        showMainButtons()
                    )
                }
                return
